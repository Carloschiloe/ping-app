...
-- -----------------------------------------------------------------------------
-- 5. commitments (núcleo del producto)
-- -----------------------------------------------------------------------------
...
--   - counterparty_user_id NO se incluye: en un compromiso 1:1, owner +
--     assigned_to ya identifican ambas partes; en uno grupal, la "contraparte"
--     es el grupo (vía conversation_id), no una persona.
...
comment on column commitments.waiting_on_user_id is 'Quién debe actuar ahora (responder, ejecutar la próxima acción, o confirmar el cierre). NULL cuando nadie está bloqueando el avance.';
...
-- -----------------------------------------------------------------------------
-- 6. commitment_events (historial append-only)
-- -----------------------------------------------------------------------------
...
create policy commitments_select on commitments for select
    ...
    using (
        ...
        or (
            ...
            and exists (
                select 1 from conversation_participants cp
                where cp.conversation_id = commitments.conversation_id and cp.user_id = auth.uid()
            )
        )
    );
            ...
            where c.id = commitment_events.commitment_id
              and (
                  ...
                  or (
                      ...
                      and exists (
                          select 1 from conversation_participants cp
                          where cp.conversation_id = c.conversation_id and cp.user_id = auth.uid()
                      )
                  )
              )
        )
