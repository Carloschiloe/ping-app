...
- **M-4 — Authorization and execution:** exact-plan binding, expiring/revocable authorizations, idempotent step claiming, tool executors, verification, and audit records. CORE WRITE FLOW PHYSICALLY CERTIFIED ON IPHONE (send_message, create_commitment, reschedule_commitment — staging commit `ca18aa5`). `respond_to_proposal` and `complete_commitment` are fully implemented and unit-tested with strengthened canonical post-write verification (see below), but have NOT yet been physically certified on iPhone. Replay/idempotency/exact-plan digest binding/actor binding CERTIFIED AT THE API/CORE LEVEL against a real local Postgres integration.
