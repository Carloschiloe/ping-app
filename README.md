...
- **M-2 — Memory:** evidence-linked memory ingestion/retrieval with sensitivity, freshness, invalidation, and canonical-fact dominance. Historical lifecycle query semantics PHYSICALLY CERTIFIED ON IPHONE (staging commit `9a46210`).
...
**Open issue found during this same physical session, NOT fixed, NOT covered by the certification above:** the query "Cuando completamos lo de entrenar?" (a *different* transition than the one physically certified for "entrenar") produced an answer narrating multiple unrelated lifecycle facts (cancelled/rejected/confirmed/accepted, plus stale historical memory statements) rather than clearly representing that no completion event exists for the resolved canonical "entrenar" commitment. At least two distinct "entrenar"-related entities exist in the real data (the canonical commitment "entrenar", cancelled, and a separate proposal "Entrenar", rejected). This is suspected to be a distinct semantic gap — Ping appears to narrate available retrieved history when a requested transition has no matching evidence, instead of explicitly representing absence of evidence for that specific transition — and requires its own root-cause audit before any fix. Tracked as open: **M-2 — Historical transition absence / cross-lifecycle contamination**.
