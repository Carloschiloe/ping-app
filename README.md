...
- **Read-path follow-up referent (branch `test/m7-read-followup-regression-20260920`, PR #3, not yet merged).** Resolves the read-side analogue of the same problem: "¿Qué pasó con Ver Spiderman?" → "¿Y cuándo lo completamos?" previously lost the entity on the second turn. The mechanism never caches an answer or a date — only a verified `{id, title}` pair from a single-citation, single-commitment `answered` response, scoped per actor+conversation, TTL 2 minutes, and it only ever *rewrites the query* (forcing a fresh canonical re-read), never substitutes a remembered answer directly. The eligibility check is a structural composition of existing signals (`isHistoricalLifecycleQuery` + the deterministic interpreter's own entity-naming signals + the existing write-action guard) rather than a single hardcoded sentence pattern, so it generalizes across lifecycle verbs and phrasings without a new hand-maintained vocabulary. Covered by unit and mocked-pipeline HTTP tests (109/109 backend files passing on that branch, `tsc` clean). **NOT a live Supabase/TLS/iPhone certification** — this is wiring-level proof only, and the branch is deliberately not merged to `codex/staging-beta` yet (stacked on PR #1, which itself is unmerged to avoid triggering staging auto-deploy).
