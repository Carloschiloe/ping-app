...
3. La pantalla móvil invalidaba el plan en el primer carácter escrito porque despachaba `SOURCE_EDITED` desde `onChangeText`. La primera contención posterior bloqueaba también las correcciones legítimas. La solución consolidada mantiene el plan mientras se redacta y permite que un envío explícito lo reemplace mediante el Core, sin ejecutar el plan anterior.
...
- La app móvil mantiene activo el plan mientras se redacta una corrección, permite enviarla explícitamente y muestra que sustituirá el plan pendiente. El reducer conserva la invalidación explícita para eventos de cambio de fuente; el Core decide si el texto es una corrección, una nueva solicitud o una salida del contexto.
