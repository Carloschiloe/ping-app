...
**Consolidation fix (2026-09-20):** `cancel_existing_commitment` was missing from `agentDialogueContinuation.service.ts`'s `TARGET_ENTITY_ELIGIBLE_OBJECTIVE_TYPES`, so a follow-up answer to "¿cuál compromiso?" (e.g. "el del jueves") after "Cancela X" against two same-titled commitments was never recognized as answering that clarification. A second, deeper defect surfaced once that was fixed: the reconciled objective only carried the resolved entity's title forward, which reproduced the same ambiguity on live re-resolution — fixed by also threading the resolved entity's own `dueAt` through `timeConstraints.rawHint`. Both reproduced end to end before being fixed; see `backend/tests/agentTurnCancelCommitment.test.ts`'s new `M-7 targetEntity clarification ... (M-9 integration)` describe block for the permanent regression coverage.
...
**Common symptoms:** a follow-up turn losing context it should have kept (check `isDialogueTrackedObjectiveType` and, specifically, `TARGET_ENTITY_ELIGIBLE_OBJECTIVE_TYPES` — a new write objective type is invisible to ALL THREE mechanisms until added there; confirmed as a real, not hypothetical, failure mode when `cancel_existing_commitment` was added for M-9 without this update, see the Commitments section's consolidation-fix note), a correction silently reusing a stale digest (should be structurally impossible — check `applyCorrection` is actually being called before the plan re-derivation, not after), an unrelated new request wrongly force-merged into an open dialogue (check `classifyExplicitEscape`), a resolved-but-still-ambiguous entity re-triggering the same clarification on the next planning pass (check that the reconciled objective carries enough live-re-derivable disambiguation — e.g. `timeConstraints.rawHint` — not just a raw title that still matches multiple candidates).
...
**Physical certification status:** NOT YET certified on iPhone (see `README.md`'s M-7 section for current status) — this domain is code-complete and covered by end-to-end tests using the real deterministic interpretation path, but has not had a physical device pass. See `docs/28-JARVIS-CONVERSATION-CERTIFICATION.md` for the full-conversation certification guide (complements the scenario-level `docs/27-M7-M8-PHYSICAL-CERTIFICATION-SCENARIOS.md`).
