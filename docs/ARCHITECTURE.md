...
- **M-2:** Added evidence-linked memory with safe ingestion, sensitivity, freshness, invalidation and canonical dominance. Memory informs but never silently overrules current facts. HISTORICAL LIFECYCLE QUERY ROUTING PHYSICALLY CERTIFIED ON IPHONE (staging commit `9a46210`): a focused historical lifecycle question ("¿Cuándo completamos/cancelamos X?") deterministically resolves to `focused_lookup` cardinality independent of the LLM's own `intent` classification (`agentInputInterpreter.service.ts`'s `HISTORICAL_LIFECYCLE_QUERY_PATTERN`, requiring the actual question form rather than the bare verb), and canonical dominance in synthesis follows structured evidence lineage (commitment/commitment_event/commitment_status-memory → commitmentId) rather than lexical title matching, so an unrelated commitment sharing a title word is never injected into the answer. Verified physically for two independent transitions ("Ver Spiderman" resolved, "entrenar" cancelled), each against real staging data with a second, unrelated same-title/same-topic commitment present in evidence and correctly excluded. A separate, NOT yet certified/NOT yet fixed issue was found in the same session (a requested transition with no matching evidence can still narrate unrelated available lifecycle history instead of representing absence of evidence) — tracked in `README.md`'s M-2 section as an open issue, out of scope for this certification.
