...
- **M-2:** Added evidence-linked memory with safe ingestion, sensitivity, freshness, invalidation and canonical dominance. Memory informs but never silently overrules current facts. HISTORICAL LIFECYCLE QUERY ROUTING PHYSICALLY CERTIFIED ON IPHONE (staging commit `9a46210`): a focused historical lifecycle question ("¿Cuándo completamos/cancelamos X?") deterministically resolves to `focused_lookup` cardinality independent of the LLM's own `intent` classification (`agentInputInterpreter.service.ts`'s `HISTORICAL_LIFECYCLE_QUERY_PATTERN`, requiring the actual question form rather than the bare verb), and canonical dominance in synthesis follows structured evidence lineage (commitment/commitment_event/commitment_status-memory → commitmentId) rather than lexical title matching, so an unrelated commitment sharing a title word is never injected into the answer. Verified physically for two independent transitions ("Ver Spiderman" resolved, "entrenar" cancelled), each against real staging data with a second, unrelated same-title/same-topic commitment present in evidence and correctly excluded.

  **Historical transition absence / cross-lifecycle contamination — CLOSED, PHYSICALLY CERTIFIED ON IPHONE (staging commit `fd98849`).** The separate issue found in the same `9a46210` session (a requested transition with no matching evidence could narrate unrelated available lifecycle history, or a true-but-non-responsive current-status fact, instead of representing absence of evidence) is now fixed and physically re-certified end-to-end: canonical target resolution moved into `agentContextBuilder.service.ts#resolveRequestedTransitionTarget`, running deterministically during context building, strictly before any LLM synthesis call — the target entity for a requested transition is never reconstructed from what the model chooses to cite. `agentResponseSynthesizer.service.ts#enforceRequestedTransitionEvidence` then verifies the requested transition against canonical evidence structurally linked to that exact resolved entity, per-claim (a claim that mentions the target but proves a different fact is discarded, never left standing as a substitute answer), and enforces absence as a first-class, Core-generated truthful outcome when no matching evidence exists — never inferred voluntarily by the LLM. A `commitment_proposal` can never satisfy or disable verification of a `commitment`'s requested transition. Physically verified for absence ("Cuando completamos lo de entrenar?" → explicit absence, 1 fuente), positive cancellation (regression-preserved), and positive completion with cross-entity disambiguation against "Spiderman el Viernes" (regression-preserved). See `README.md`'s M-2 section for the full physical transcript.
...
- **M-7 durable boundary:** General `/agent/turn` now has an opt-in local/staging boundary with admission, idempotent replay, request-scoped dialogue restoration and atomic checkpoint commit. The default legacy path remains unchanged until local integration and controlled staging certification complete. Tablet is not a separate agent; it remains a future adapter over this same Core.
